...
export default function Modal({open, onClose, photo}) {
    return (
        <div className={`absolute inset-0 flex justify-center items-center ${open ? '' : 'pointer-events-none'}`}>
            ...
            <div className={`fixed bottom-4 top-28 w-auto shadow-lg backdrop-blur-md p-4 ${open ? '' : 'pointer-events-none opacity-0'}`}>
                ...
                <Image src={photo} alt="photo" height={400} width={400} className={"h-5/6 w-auto opacity-100"}/>
            </div>
        </div>
    )
}